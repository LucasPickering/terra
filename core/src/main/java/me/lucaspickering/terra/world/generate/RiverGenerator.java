...
//            .filter(t -> t.getRunoffTraversed() >= RIVER_THRESHOLD)
//            .collect(Collectors.toCollection(TileSet::new));
//
//        while (!riverTiles.isEmpty()) {
//            Tile firstTile = riverTiles.stream()
//                .max(Comparator.comparingDouble(Tile::getRunoffTraversed))
...
//        }
    ...
    private Map.Entry<Direction, Tile> getNextTile(Tile tile) {
        ...
        nextTile = adjTiles.entrySet().stream()
            ...
            .max(Comparator.comparingDouble(p -> p.getValue().getRunoffTraversed()))
            .orElse(null);
        if (nextTile != null) {
            return nextTile;
        }
