 ...
 * follows: <ul> <li>Drop x liters of water on each land tile</li> <li>For each tile, move its water
 * onto each adjacent tile that is lower than it; apply this to tiles in descending order of
 * elevation</li> <li>If there is no where for a tile's water to go, and its water level hits some
 * threshold, that tile becomes a lake</li> <li>At each step keep track of how much water has moved
 * across each tile; if this hits some threshold, that tile becomes a river</li> </ul>
 */
public class FreshWaterStepper extends Stepper {
    ...
    public void step() {
        ...
        logger().fine(String.format("Runoff iteration took %d ms", elapsedTime));
    }
...
//                                                t1.elevation())) // Sort by desc elev
    ...
    private void logStatus(Tile tile, TileSet adjTiles, double targetWaterElev) {
        ...
        logger().finest(String.format("  Calculated target water level: [%f]",
                                                 targetWaterElev));
